...
if [[ -n $GH_TOKEN && -n $GITLAB_ACCESS_TOKEN ]]; then
    git ls-remote --heads --exit-code https://x-access-token:$GH_TOKEN@github.com/"$github_user_name"/"$github_repo_name".git >/dev/null
    ...
    git push --mirror https://x-access-token:$GH_TOKEN@github.com/"$github_user_name"/"$github_repo_name".git
else
